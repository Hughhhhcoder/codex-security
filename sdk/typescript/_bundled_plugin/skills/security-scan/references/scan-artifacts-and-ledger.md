...
- For deleted revision paths, read the complete selected base blob with `git --no-replace-objects show "<base>:<path>"`. When a local-patch preview distinguishes the staged Git index from the worktree, review both `git --no-replace-objects show ":<path>"` and the current file in full; inspect both staged and worktree submodule pins when they differ.
