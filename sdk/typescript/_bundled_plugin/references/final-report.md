...
Supply semantic coverage as `{ completeness, surfaces, explicitExclusions, deferred, openQuestions? }`, with each surface using the actual `label` and one existing `disposition`. Completeness is relative to the exact requested review and authorized scope. A finished review of selected paths is a complete scoped scan, not a partial repository scan. Reserve deferred items and `needs_follow_up` surfaces for unfinished requested work or essential unresolved in-scope proof gaps. They still require `partial` coverage; preserve their real reasons and supporting context. Record optional additional assurance, deployment confirmation, or broader investigations in `openQuestions`, and record the agreed validation method, runtime status, and limitations in `scan.scope`. Those facts do not by themselves make a completed source review partial. Never move a real blocker to open questions or invent an exclusion to claim completion. Each deferred item needs a meaningful reason; preserve any existing `id` or `candidateId`. The workbench derives a missing ID from its candidate identity or stable deferred-work details. Open questions may be nonempty strings or `{ question, followUpPrompt? }` objects. The workbench derives target and scope metadata, scope include and exclude paths, coverage mode and inventory strategy, finding identities and fingerprints, and surface IDs. Do not put those workbench-owned values or top-level coverage receipt references into the semantic draft.
...
The generated `## Incomplete Requested Work` section identifies blocking deferred work and unresolved in-scope surfaces. Keep optional additional assurance separate in `## Open Questions And Follow Up`. Include that section only when there is a concrete useful next investigation, including for an otherwise complete scoped scan. Use copyable prompts grounded in the actual selected source and findings; do not invent follow-ups or broaden the current scan.
