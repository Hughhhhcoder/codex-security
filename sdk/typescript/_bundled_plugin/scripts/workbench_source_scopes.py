...
def filesystem_alias(selected: Path, candidate: Path) -> bool:
    if normalized_path_component(selected.name) != normalized_path_component(
        candidate.name
    ):
        return False
    try:
...
def target_tree(target: Path, revision: str) -> tuple[Path, str] | None:
    ...
    raw_tree = offline_git_bytes(
        repository, "rev-parse", "--verify", f"{revision}^{{tree}}"
    )
...
def load_source_scopes(
    ...
    legacy_files = (
        legacy_recipe.get("_codexSecurityFileScopes")
...
def source_object_for_path(
    ...
    if entry is None:
        ...
        if selected is not None:
            entry = existing_tree_path(
                repository, scope["objectId"], relative, selected
            )
