...
test("reads native workbench history larger than four MiB", async () => {
  const run = promisify(execFile);
  const executeWorkbench = await loadExecuteWorkbench(
    async (command, args, options) => {
      const result = await run(command, args, { ...options, encoding: "utf8" });
      expect(options.maxBuffer).toBe(Infinity);
      return result;
    },
    "-e",
  );
  const script = [
    'const paths = Array.from({ length: 266 }, (_, i) => "src/" + "component/".repeat(4) + i + "/source.ts");',
    'process.stdout.write(JSON.stringify({ scans: Array.from({ length: 320 }, (_, i) => ({ scanId: "scan-" + i, scopePaths: paths })) }));',
  ].join("\n");
  const result = await executeWorkbench(process.execPath, [script]);
  const scans = result["scans"] as Array<{
    scanId: string;
    scopePaths: string[];
  }>;
  ...
  expect(scans).toHaveLength(320);
  expect(scans[319]?.scanId).toBe("scan-319");
  expect(scans[319]?.scopePaths).toHaveLength(266);
  expect(scans[319]?.scopePaths[265]).toBe(
    `src/${"component/".repeat(4)}265/source.ts`,
  );
});
