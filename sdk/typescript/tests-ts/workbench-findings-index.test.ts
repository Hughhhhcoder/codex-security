...
const nestedDirectoryScanProbe = [
  ...
  "    output = {}",
  "    unverified_reads = []",
  "    unverified_matching = list_unmatched_scan_pairs(connection, argparse.Namespace(repository=str(root), force=False), backfill_finding_details=lambda *_: None, read_coverage=lambda scan: unverified_reads.append(scan['id']) or {})",
  "    output['unverifiedRegisteredMatching'] = {'scanCount': unverified_matching['scanCount'], 'coverageReads': unverified_reads}",
  ...
  "    for owner in (moved_target, current_target, portable_target):",
  "        owner_path = connection.execute('SELECT current_path FROM security_targets WHERE id = ?', (owner,)).fetchone()['current_path']",
  "        owner_metadata = pathlib.Path(owner_path).stat()",
  "        connection.execute('UPDATE scans SET target_device = ?, target_inode = ? WHERE target_id = ? AND target_device IS NULL AND target_inode IS NULL', (serialize_filesystem_identity(owner_metadata.st_dev), serialize_filesystem_identity(owner_metadata.st_ino), owner))",
  ...
  "    output['unownedExplicitComparison'] = _same_repository(targetless_original, targetless_original, require_ownership=True)",
  ...
  "    original_metadata = original_clone.stat()",
  "    owned_original = connection.execute('SELECT ? AS target_id, ? AS target_path, ? AS target_device, ? AS target_inode', (portable_target, str(original_clone), serialize_filesystem_identity(original_metadata.st_dev), serialize_filesystem_identity(original_metadata.st_ino))).fetchone()",
  "    owned_related = connection.execute('SELECT * FROM scans WHERE id = ?', ('related-worktree-scan',)).fetchone()",
  "    output['registeredSiblingIdentity'] = [_same_repository(owned_original, owned_related, require_ownership=True), _same_repository(owned_related, owned_original, require_ownership=True)]",
  ...
  "    replaced_worktree_scan = connection.execute('SELECT ? AS target_id, ? AS target_path, ? AS target_device, ? AS target_inode', ('previous-worktree-owner', str(related_clone), serialize_filesystem_identity(related_metadata.st_dev + 1), serialize_filesystem_identity(related_metadata.st_ino))).fetchone()",
  "    output['reusedLinkedWorktreeIdentity'] = _same_repository(replaced_worktree_scan, connection.execute('SELECT * FROM scans WHERE id = ?', ('portable-scan',)).fetchone())",
...
describe("workbench findings index", () => {
  ...
  test("finds unversioned directory scans from nested subdirectories", () => {
    ...
    expect(JSON.parse(new TextDecoder().decode(result.stdout))).toEqual({
      ...
      unverifiedRegisteredMatching: { scanCount: 0, coverageReads: [] },
      ...
      reusedLinkedWorktreeIdentity: false,
  ...
  test("casefolds scan-specific searches and rejects unverified checkout identities", () => {
    const python =
      Bun.which("python3") ?? Bun.which("python") ?? Bun.which("py");
    expect(python).not.toBeNull();
    if (python === null)
