  ...
  test("does not follow repository parent symlinks during inventory or snapshots", async () => {
    ...
    const result = runPythonProbe(
      [
        ...
        "    workbench.git_bytes = lambda *_: (unicode_alias.name + '/public.py\\0').encode()",
        "    unicode_accepted = len(workbench.git_directory_snapshot_paths(unicode_directory)) == 1",
        "workbench.git_worktree_context = lambda _: (repository, '.')",
        ...
        "print(json.dumps({'inventories': inventories, 'externalReads': reads, 'snapshotRejected': snapshot_rejected, 'missingSkipped': missing_skipped, 'selectedPaths': selected_paths, 'parentResolutions': len(parent_resolutions), 'unicodeAccepted': unicode_accepted, 'directLinkRejected': direct_link_rejected}))",
      ].join("\n"),
      repository,
