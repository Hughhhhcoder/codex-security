...
describe("canonical scan contract", () => {
  ...
  test("binds immutable diff snapshots to authoritative Git revisions", async () => {
    const python =
    ...
    const probe = [
      ...
      "repository = Path(sys.argv[2]).resolve() / 'repository'",
      "repository.mkdir()",
      "def git(*args):",
      "    return subprocess.run(['git', *args], cwd=repository, capture_output=True, text=True, check=True).stdout.strip()",
      "git('init', '-q')",
      "git('config', 'user.name', 'Codex Security')",
      "git('config', 'user.email', 'codex-security@example.test')",
      ...
      "trees = tuple(git('rev-parse', revision + '^{tree}') for revision in (base, head))",
      "foreign = Path(sys.argv[2]).resolve() / 'foreign'",
      "subprocess.run(['git', 'clone', '-q', str(repository), str(foreign)], capture_output=True, text=True, check=True)",
      "subprocess.run(['git', '-C', str(foreign), 'replace', head, base], capture_output=True, text=True, check=True)",
      "inventory = Path(sys.argv[2]).resolve() / 'inventory.jsonl'",
      "rank_command = [sys.executable, str(Path(sys.argv[1]) / 'generate_rank_input.py'), 'make-diff-rank-input', '--repo', str(repository), '--base', base, '--head', head, '--out', str(inventory)]",
      "foreign_environment = {**os.environ, 'GIT_DIR': str(foreign / '.git'), 'GIT_WORK_TREE': str(foreign)}",
      "subprocess.run(rank_command, env=foreign_environment, capture_output=True, text=True, check=True)",
      "assert [json.loads(row)['path'] for row in inventory.read_text().splitlines()] == ['source.ts']",
      "connection = sqlite3.connect(':memory:')",
      "connection.row_factory = sqlite3.Row",
      "results = {}",
      "for kind in ('commit', 'range'):",
      "    target = workbench.require_diff_target(repository, kind, base, head, None)",
      "    expected = 'codex-security-snapshot/v1:sha256:' + hashlib.sha256(chr(0).join((kind, base, head, *trees, '', 'true', 'unset')).encode()).hexdigest()",
      ...
      "    binding = workbench.workbench_completion_binding(scan, '2026-01-01T00:00:01Z')",
      "    assert binding['target']['snapshotDigest'] == expected",
      "    manifest = {'scan': {'id': 'scan', 'target': {'kind': 'git_diff', **binding['target']}, 'scope': binding['scope']}}",
      "    workbench.verify_manifest_binding(scan, manifest)",
      "    manifest['scan']['target']['snapshotDigest'] = 'codex-security-snapshot/v1:sha256:' + '0' * 64",
      "    try:",
      "        workbench.verify_manifest_binding(scan, manifest)",
      "    except SystemExit:",
      "        pass",
      "    else:",
      "        raise SystemExit('accepted an invented immutable diff snapshot')",
      "    results[kind] = expected",
      ...
      "    replacement = git('commit-tree', replacement_tree, '-p', base, '-m', 'replacement')",
      "    git('replace', head, replacement)",
      "    assert git('rev-parse', head) == head",
      "    replaced = workbench.require_diff_target(repository, 'range', base, head, None)",
      "    assert replaced['contentDigest'] != results['range']",
      "    try:",
      "        workbench.workbench_completion_binding(registered_scan, '2026-01-01T00:00:02Z')",
      "    except SystemExit:",
      "        pass",
      "    else:",
      "        raise SystemExit('accepted Git objects replaced during the scan')",
      "    results['replacement'] = replaced['contentDigest']",
      "    git('replace', '-d', head)",
      "    git('replace', trees[1], replacement_tree)",
      "    assert git('rev-parse', head + '^{tree}') == trees[1]",
      ...
      "    environment_disabled = workbench.require_diff_target(repository, 'range', base, head, None)",
      "    assert environment_disabled['contentDigest'] != tree_replaced['contentDigest']",
      "    results['environment_disabled'] = environment_disabled['contentDigest']",
      "    del os.environ['GIT_NO_REPLACE_OBJECTS']",
